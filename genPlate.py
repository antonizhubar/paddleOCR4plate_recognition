...
class GenPlate:
    ...
    def genBatch(self, batchSize, outputPath, label_file, size):
        if (not os.path.exists(outputPath)):
            os.mkdir(outputPath)
        outfile = open(label_file,'w')
        for i in range(batchSize):
            plateStr,plate = self.genPlateString(-1,-1)
            #print(plateStr,plate)
            img = self.generate(plateStr)
            img = cv2.resize(img, size)
            #cv2.imwrite(outputPath + "/" + str(i).zfill(2) + ".jpg", img)
            #outfile.write("272 72 " + str(i).zfill(2) + '.jpg ')
            cv2.imwrite(outputPath + "/" + plateStr + ".jpg", img)
            outfile.write("272 72 " + plateStr + '.jpg ')
            plate_indexs = [str(index[x]) for x in plate]
            outfile.write(','.join(plate_indexs)+"\n")

if __name__=='__main__':
    G = GenPlate("./font/platech.ttf",'./font/platechar.ttf',"./NoPlates")
    #G.genBatch(100,range(31,65),"./plate_100",(272,72))
    image_dir = 'test_data/test_images2'
    label_file = 'test_data/test_list2'
    G.genBatch(int(sys.argv[1]), image_dir, label_file, (272,72))
